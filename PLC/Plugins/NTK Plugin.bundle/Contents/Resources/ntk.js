...
// Define the modResolve function to resolve modules (used by require)
Duktape.modResolve = function ( requestedID, parentID )
{
	var path = requestedID;
	if( parentID )
		path = File.normalize( File.parent( parentID ) + requestedID );
	...
	if( !path )
		path = app.resolvePath( requestedID + File.separator + 'index.js' );
	
	if( !path )
		throw new Error( 'Module not found: ' + requestedID );
	
	return path;
}

// Define the modLoad function to load modules (used by require)
Duktape.modLoad = function ( resolvedID, exports, module )
{
	var path = resolvedID;
	...
	{
		if( File.extension( resolvedID ) == 'json' ) {
			// Parse the JSON and assign it to module.exports
...
Application.prototype.resolvePath = function( filename )
{
	...
	var path = File.normalize( __dirname + filename );
	if( File.isFile( path ) )
		return path;
		
	for( var i in this.searchPaths )
	{
		path = File.normalize( this.searchPaths[i] + filename );
		if( File.isFile( path ) )
			return path;
	}
...
{
	this.name = this.eval4D( 489, 'Structure file', '(*)' );
